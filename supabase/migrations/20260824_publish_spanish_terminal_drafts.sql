-- Publication contrôlée des brouillons complets d’espagnol Terminale A1/A2/C/D.
-- Le périmètre est figé à 32 leçons non actives avec un contenu non vide.
do $publish_spanish_terminal_drafts_do$
declare
  target record;
...
begin
  ...
  if target_count <> 32 then
    raise exception 'Publication refusée : 32 brouillons complets d’espagnol Terminale étaient attendus, % trouvés.', target_count;
  end if;
  ...
  if incomplete_count <> 0 then
    raise exception 'Publication refusée : % brouillon(s) espagnols sont incomplets.', incomplete_count;
  end if;
...
end $publish_spanish_terminal_drafts_do$;
