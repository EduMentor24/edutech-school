  ...
  return (
    <AppScreen withPadding={false}>
      <View style={styles.container}>
        ...
        {isLoading ? <View style={styles.loading}><ActivityIndicator color={colors.primary} /><Text style={styles.loadingText}>Chargement du dictionnaire…</Text></View> : results.length === 0 ? <EmptyState icon="menu-book" title={query ? "Aucun terme trouvé" : "Aucune fiche disponible"} description={query ? "Essayez un autre mot ou vérifiez son orthographe." : "Le corpus complet est disponible localement et peut être consulté hors connexion."} /> : <FlatList data={results} keyExtractor={(item) => item.id} showsVerticalScrollIndicator={false} contentContainerStyle={styles.list} renderItem={({ item }) => <DictionaryCard entry={item} expanded={expandedId === item.id} onToggle={() => setExpandedId(expandedId === item.id ? null : item.id)} colors={colors} styles={styles} />} />}
      </View>
    </AppScreen>
  );
}
