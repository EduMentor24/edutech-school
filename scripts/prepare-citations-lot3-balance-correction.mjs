...
const migrationPath = resolve("supabase/migrations/20260820_citations_lot3_equilibre_correction.sql");
...
const rows = corrections.map(([subject, currentText, author, theme, correctedText]) => `      (${quote(subject)},${quote(currentText)},${quote(author)},${quote(theme)},${correctedText === null ? "null" : quote(correctedText)})`).join(",\n");

const sql = `-- Ajustement des thèmes du lot Citations 3 après contrôle d’équilibre.
