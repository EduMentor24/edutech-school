...
export type AdminExercise = { id: string; subjectId: string; levelId: string; seriesId: string; seriesName: string; chapterId: string; lessonId: string; title: string; statement: string; contentMarkdown: string | null; correctionMarkdown: string | null; exerciseType: ExerciseType; difficulty: ExerciseDifficulty | null; estimatedDurationMinutes: number | null; displayOrder: number; isPublished: boolean; isActive: boolean; isTestData: boolean; subjectName: string; chapterTitle: string; lessonTitle: string; questionCount: number; attemptCount: number; createdAt: string; updatedAt: string };
