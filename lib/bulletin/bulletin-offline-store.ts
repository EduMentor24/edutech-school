...
import { AssessmentType, BulletinTerm, calculateCoefficientMatrixTotal, calculateSubjectAverage, calculateTermAverage } from "./bulletin-model";
...
export type BulletinOfflineSnapshot = { version: 1 | 2; subjects: BulletinSubject[]; subjectsBySchoolYear?: Record<string, BulletinSubject[]>; grades: LocalBulletinGrade[]; lastSyncedAt: string | null };
...
const emptySnapshot = (): BulletinOfflineSnapshot => ({ version: 2, subjects: [], subjectsBySchoolYear: {}, grades: [], lastSyncedAt: null });
...
export function buildLocalBulletinSummary(snapshot: BulletinOfflineSnapshot, schoolYear: string, term: BulletinTerm): BulletinSummary {
  const sourceSubjects = snapshot.subjectsBySchoolYear?.[schoolYear] ?? snapshot.subjects;
  const subjects = sourceSubjects.map((subject) => { const grades = snapshot.grades.filter((grade) => grade.offeringId === subject.offeringId && grade.schoolYear === schoolYear && grade.term === term); return { ...subject, average: calculateSubjectAverage(grades.map((grade) => ({ grade: grade.grade, maxGrade: grade.maxGrade, assessmentCoefficient: grade.assessmentCoefficient, includeInAverage: grade.includeInAverage }))), noteCount: grades.length }; });
  const total = calculateTermAverage(subjects.map((subject) => ({ average: subject.average, coefficient: subject.coefficient, coefficientVerified: subject.coefficientVerified, isOptional: subject.isOptional })));
  const coefficientMatrixTotal = calculateCoefficientMatrixTotal(subjects.map((subject) => ({ coefficient: subject.coefficient, coefficientVerified: subject.coefficientVerified, isOptional: subject.isOptional })));
  return { schoolYear, term, subjects, termAverage: total.average, coefficientMatrixTotal, includedSubjectCount: total.includedSubjectCount, excludedUnverifiedCoefficientCount: total.excludedUnverifiedCoefficientCount };
}
...
export async function mergeRemoteBulletinSnapshot(studentId: string, remoteSubjects: BulletinSubject[], remoteGrades: BulletinGrade[], schoolYear: string, term: BulletinTerm) {
  ...
  const snapshot: BulletinOfflineSnapshot = { version: 2, subjects: remoteSubjects, subjectsBySchoolYear: { ...(current.subjectsBySchoolYear ?? {}), [schoolYear]: remoteSubjects }, grades: [...gradeById.values()], lastSyncedAt: now() };
...
export async function saveLocalBulletinGrade(studentId: string, input: BulletinGradeInput, gradeId?: string) {
  const [snapshot, queue] = await Promise.all([readBulletinSnapshot(studentId), readBulletinQueue(studentId)]); const subject = (snapshot.subjectsBySchoolYear?.[input.schoolYear] ?? snapshot.subjects).find((item) => item.offeringId === input.offeringId); if (!subject) throw new Error("La matière n’est pas disponible dans le cache local du Bulletin.");
