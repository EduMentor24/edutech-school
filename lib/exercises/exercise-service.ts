...
export type ExerciseAttempt = {
  ...
  correctionMarkdown?: string | null;
};
export type ExerciseAttemptResult = ExerciseAttempt & {
  title: string;
  questions: Array<
    ExerciseQuestion & {
      answer: unknown;
...
function catalogFrom(row: RpcRow): ExerciseCatalogItem {
  return {
    ...
    difficulty: (row.difficulty as ExerciseDifficulty | null) ?? null,
    estimatedDurationMinutes: nullableNumeric(row.estimated_duration_minutes),
...
export async function getExerciseCatalog(
  ...
  return readPedagogicalLocalFirst(
    ...
    resource,
    async () => {
      const { data, error } = await supabase.rpc("get_my_exercise_catalog", {
...
export async function getExerciseDetail(
  ...
  return readPedagogicalLocalFirst(
    ...
    async () => {
      const { data, error } = await supabase.rpc("get_my_exercise_detail", {
      ...
      const { data: version, error: versionError } = await supabase.rpc(
        "get_my_exercise_content_version",
        { p_exercise_id: exerciseId },
      );
      ...
      return {
        ...
        exerciseType: row.exercise_type as ExerciseType,
        difficulty: (row.difficulty as ExerciseDifficulty | null) ?? null,
        estimatedDurationMinutes: nullableNumeric(
        ...
        questions: rawQuestions.map((question: any) => ({
          ...
          questionType: question.question_type as ExerciseType,
          promptMarkdown: String(question.prompt_markdown ?? ""),
          options: asOptions(question.options),
          displayOrder: numeric(question.display_order),
        })),
      };
    },
...
function attemptFrom(row: RpcRow): ExerciseAttempt {
  return {
    attemptId: String(row.attempt_id),
    exerciseId: String(row.exercise_id),
    status: row.status as ExerciseAttemptStatus,
    startedAt: String(row.started_at),
    score: nullableNumeric(row.score),
    ...
    correctionMarkdown: (row.correction_markdown as string | null) ?? null,
  };
}
...
export async function submitExerciseAttempt(
  ...
  if (error) throw new Error(message(error));
  const row = one(data);
  if (!row) throw new Error("La tentative n’a pas pu être soumise.");
  return attemptFrom(row);
}

export async function getExerciseAttemptResult(
  attemptId: string,
): Promise<ExerciseAttemptResult | null> {
  const { data, error } = await supabase.rpc("get_my_exercise_attempt_result", {
    p_attempt_id: attemptId,
  });
  if (error) throw new Error(message(error));
  const row = one(data);
  ...
  return {
    ...
    questions: rawQuestions.map((question: any) => ({
      ...
      isCorrect:
        typeof question.is_correct === "boolean" ? question.is_correct : null,
    })),
  };
}

export async function getMyExerciseHistory(): Promise<ExerciseHistoryItem[]> {
