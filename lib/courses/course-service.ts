...
import { pedagogicalCacheContextFromProfile, readPedagogicalLocalFirst, type PedagogicalCacheContext } from "@/lib/offline/pedagogical-cache";
...
type CourseCacheOptions = { cacheContext?: PedagogicalCacheContext | null };
type LessonOptions = { includeInactive?: boolean; cacheContext?: PedagogicalCacheContext | null };
...
export async function getCoursesForProfile(profile: StudentProfile): Promise<CourseSubject[]> {
  ...
  return readPedagogicalLocalFirst(cacheContext, "catalog/subjects", () => fetchCoursesForProfile(profile));
}

async function fetchCoursesForProfile(profile: StudentProfile): Promise<CourseSubject[]> {
...
export async function getCourseOffering(offeringId: string, options: CourseCacheOptions = {}): Promise<CourseOffering | null> {
  return readPedagogicalLocalFirst(options.cacheContext, `offering/${segment(offeringId)}`, async () => {
...
export async function getChaptersForOffering(offeringId: string, options: CourseCacheOptions = {}): Promise<CourseChapter[]> {
  return readPedagogicalLocalFirst(options.cacheContext, `offering/${segment(offeringId)}/chapters`, async () => {
...
export async function getChapter(chapterId: string, options: CourseCacheOptions = {}): Promise<CourseChapter | null> {
  return readPedagogicalLocalFirst(options.cacheContext, `chapter/${segment(chapterId)}`, async () => {
...
export async function getLessonsForChapter(chapterId: string, options: LessonOptions = {}): Promise<CourseLesson[]> {
...
export async function getLesson(lessonId: string, options: LessonOptions = {}): Promise<CourseLesson | null> {
