...
export async function getChaptersForOffering(offeringId: string): Promise<CourseChapter[]> {
  const { data, error } = await supabase.from("chapters").select("id,title,description,display_order").eq("subject_offering_id", offeringId).order("display_order", { ascending: true });
...
export async function getChapter(chapterId: string): Promise<CourseChapter | null> {
  const { data, error } = await supabase.from("chapters").select("id,title,description,display_order").eq("id", chapterId).maybeSingle();
...
export async function getLessonsForChapter(chapterId: string): Promise<CourseLesson[]> {
  const { data, error } = await supabase.from("lessons").select("id,chapter_id,title,description,content,display_order").eq("chapter_id", chapterId).order("display_order", { ascending: true });
...
export async function getLesson(lessonId: string): Promise<CourseLesson | null> {
  const { data, error } = await supabase.from("lessons").select("id,chapter_id,title,description,content,display_order").eq("id", lessonId).maybeSingle();
