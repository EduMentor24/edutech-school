...
import { diagnoseGeminiFailure, isSupportedMentorAttachment, mentorConversationStorageKey, mentorKeyMask, mentorMaxAttachmentBytes, mentorStudentMessage } from "../lib/mentor/mentor-policy";
...
describe("règles de sécurité du Mentor IA", () => {
  ...
  it("fournit à l’administrateur un diagnostic sûr de clé, quota ou modèle", () => { expect(diagnoseGeminiFailure(403, "PERMISSION_DENIED")).toMatchObject({ status: "invalid", code: "INVALID" }); expect(diagnoseGeminiFailure(429, "RESOURCE_EXHAUSTED")).toMatchObject({ status: "quota", code: "QUOTA" }); expect(diagnoseGeminiFailure(404, "NOT_FOUND")).toMatchObject({ status: "unavailable", code: "UNAVAILABLE" }); });
});
