...
const migration = JSON.parse(
  readFileSync(resolve(root, "commande14_1_restructure.json"), "utf8"),
) as { query: string };
const englishMigration = JSON.parse(
  readFileSync(resolve(root, "commande14_1_english_units.json"), "utf8"),
) as { query: string };
const catalogueService = readFileSync(
  resolve(root, "lib/courses/course-service.ts"),
  "utf8",
);
...
describe("Commande 14.1 — Restructuration manuelle Première C/D", () => {
  ...
  it("supprime seulement les chapitres/leçons C/D des matières explicitement corrigées", () => {
    expect(migration.query).toContain(
    ...
    expect(migration.query).not.toContain(
      "'Sciences de la Vie et de la Terre (C)'",
    );
    expect(migration.query).not.toContain(
      "'Sciences de la Vie et de la Terre (D)'",
    );
  });

  it("déclare exactement les 17 leçons de Mathématiques Première C", () => {
    const section = migration.query.slice(
      migration.query.indexOf("('C', 'Mathématiques', 1, 1"),
      migration.query.indexOf("('D', 'Mathématiques', 1, 1"),
    );
  ...
  it("déclare exactement les 15 leçons de Mathématiques Première D", () => {
    const section = migration.query.slice(
      migration.query.indexOf("('D', 'Mathématiques', 1, 1"),
      migration.query.indexOf("('C', 'Physique-Chimie', 1, 1"),
    );
  ...
  it("déclare les cinq chapitres et 28 leçons de Physique-Chimie Première C", () => {
    const section = migration.query.slice(
      migration.query.indexOf("('C', 'Physique-Chimie', 1, 1"),
      migration.query.indexOf("('D', 'Physique-Chimie', 1, 1"),
    );
  ...
  it("déclare les cinq chapitres et 26 leçons de Physique-Chimie Première D", () => {
    const section = migration.query.slice(
      migration.query.indexOf("('D', 'Physique-Chimie', 1, 1"),
      migration.query.indexOf("('C', 'Français', 1, 1"),
    );
  ...
  it("conserve les structures communes de Français, Espagnol, Philosophie et Histoire-Géographie pour C/D", () => {
    expect(
      occurrences(migration.query, "'CHAPITRE 9 — EXPRESSION ÉCRITE'"),
    ).toBe(2);
    expect(
      occurrences(
        migration.query,
        "'CHAPITRE 6 — La technique d’expression orale ou écrite'",
      ),
    ).toBe(2);
    expect(
      occurrences(migration.query, "'CHAPITRE 3 — HISTOIRE DE LA PHILOSOPHIE'"),
    ).toBe(2);
    expect(
      occurrences(
        migration.query,
        "'CHAPITRE 4 — Le processus de la mondialisation'",
      ),
    ).toBe(2);
  });
  ...
  it("force les leçons corrigées à rester vides et inactives", () => {
    ...
    expect(englishMigration.query).toContain(
      "SELECT c.id, unit_ref.title, '', '', 1, false, false",
    );
  ...
  it("conserve l’invisibilité catalogue des offres, chapitres et leçons non publiés/inactifs", () => {
    ...
    expect(catalogueService).toMatch(
      /chapters\.filter\(\s*\(chapter: \{ is_active\?: boolean \}\) => chapter\.is_active,?\s*\)/,
    );
